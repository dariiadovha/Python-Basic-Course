...
def dict_check(compiled_dict):
    frequency = 100
    pattern = re.compile(' ''-' '[a-zA-Z]')
    while frequency != len(compiled_dict):
        for el in compiled_dict:
            matched_phone = pattern.match(compiled_dict.get('Phone'))
            matched_card = pattern.match(compiled_dic.get('Card'))
            if matched_phone or matched_card is True:
                with open('cardholders_errors.csv', 'w', newline = '')as csvfile:
                    header = ['Name', 'Phone', 'Exp_day', 'Card']
                    writer = csv.DictWriter(csvfile, fieldnames = header)
                    writer.writeheader()
                    writer.writerow({'Name': compiled_dict.get('Name'),'Phone': compiled_dict.get('Phone'), 'Exp_day': compiled_dict.get('Exp_day'), 'Card':compiled_dict.get('Card')})
            else:
                with open('cardholders_errors.csv', 'w', newline = '')as csvfile:
                    header = ['Name', 'Phone', 'Exp_day', 'Card']
                    writer = csv.DictWriter(csvfile, fieldnames = header)
